function [x_u_series,RMSE,cov_pos_j] = IPLF_comp(Nsteps,x_0,P_0,R,Q,F,N_x,x_truth,lambda,N_it,y_measure)
...
%%   IPLF Update
    ...
    %% IPLF Prediction
        [X,W] = sigma_point_IPLF_comp(mean_update, var_update, N_x,lambda);% X and W are matrix
        X_hat=F*X;
        var_pred=zeros(3,3);
        x_pred=[0;0;0];
        for j=1:2*N_x+1
...
%         for j=1:7
%         var_pred=var_pred+W(j)*X_hat(:,j)*X_hat(:,j)';
%         x_pred=x_pred+W(j)*X_hat(:,j);
%         end
%         x_pred=X_hat*W;% 3*7 %7*1
%         var_pred=var_pred-x_pred*x_pred'+Q; 
