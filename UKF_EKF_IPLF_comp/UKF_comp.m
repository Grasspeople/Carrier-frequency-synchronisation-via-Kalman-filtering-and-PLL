function [x_u_series,RMSE,P_u] = UKF_comp(Nsteps,x_ini,P,R,Q,F,y_measure,N_x,x_truth)
...
for k=1:Nsteps   
     ...
     %Update
    [X,W_m,W_c] = sigma_point_UKF_comp(x_pred, P_pred, N_x);
    for i=1:2*N_x+1
        ob_pred_point=[cos(X(1,i));sin(X(1,i))];
    ...
    %Prediction
    [X,W_m,W_c] = sigma_point_UKF_comp(x_u, P_u, N_x);
    X_hat=F*X;
    x_pred=X_hat*W_m;
    P_pred=zeros(3,3);
    for i=1:2 * N_x + 1
