function [x_u_series,RMSE,Pk,P_pro] = IPLF_pro(Nsteps,x_0,P_0,R,Q,F,N_x,x_truth,lambda,N_it,y_measure)
...
%%   IPLF Update
    ...
    cov_pos_j=Pk;
    for p=1:N_it 
        [X,W_m,W_c] = generate_sigma_point_IPLF_prob(mean_pos_j, cov_pos_j, N_x,lambda);
    ...
    %% IPLF Prediction
        [X,W_m,W_c] = generate_sigma_point_IPLF_prob(mean_j, P_j, N_x,lambda);% X and W are matrix
        X_hat=F*X;
        var_pred=zeros(3,3);
        x_pred=[0;0;0];
        for j=1:7
