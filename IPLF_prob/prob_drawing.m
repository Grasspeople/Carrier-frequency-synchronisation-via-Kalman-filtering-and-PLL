...
N_it=1;
Nsteps=1;
[x_u_series_1,RMSE,Pk_1,P_pro_1] = IPLF_pro(Nsteps,x_0,P_0,R,Q,F,N_x,x_truth,lambda,N_it,y_measure);
% disp(Pk_1);
...
N_it=5;
Nsteps=1;
[x_u_series_2,RMSE,Pk_2,P_pro_2] = IPLF_pro(Nsteps,x_0,P_0,R,Q,F,N_x,x_truth,lambda,N_it,y_measure);
% disp(Pk_2);
sigma = sqrt(Pk_2(1,1));
mu=x_u_series_2(1,Nsteps);
x = linspace(mu-3*sigma, mu+3*sigma, 1000);
pdf_values = normpdf(x, mu, sigma);
plot(x, pdf_values,'b', 'linewidth', 1);
hold on

N_it=5;
Nsteps=2;
[x_u_series_5,RMSE,Pk_5, P_pro_5] = IPLF_pro(Nsteps,x_0,P_0,R,Q,F,N_x,x_truth,lambda,N_it,y_measure);
% disp(Pk_5);
sigma = sqrt(P_pro_5(1,1));
